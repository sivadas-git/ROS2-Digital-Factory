...
public class RobotLogger : MonoBehaviour
{
    ...
    void Start()
    {
        logPath = Path.Combine(Application.dataPath, "robot_rtt_log.csv");

        if (!File.Exists(logPath))
        {
            File.WriteAllText(logPath, "Timestamp,RequestID,RTT_ms,JointAngles\n");
        }
    ...
    void StartServer()
    {
        try
        {
            ...
            listener.Start();
            isRunning = true;
            logQueue.Enqueue("✅ Robot RTT Server started on port " + port);
            ...
            while (isRunning && client.Connected)
            {
                ...
                if (reply.StartsWith("REQ"))
                {
                    ...
                    if (parts.Length == 2 && requestTimestamps.TryGetValue(parts[0], out DateTime sentTime))
                    {
                        ...
                        logQueue.Enqueue($"📡 RTT ({parts[0]}): {rttMs} ms | Joint Angles: {parts[1]}");
            ...
            logQueue.Enqueue("🚪 Robot RTT connection closed.");
        }
        catch (Exception e)
        {
            logQueue.Enqueue("❌ Robot RTT server error: " + e.Message);
        }
    }
}
