...
public class PLCLogger : MonoBehaviour
{
    public int port = 31007;
    ...
    void Start()
    {
        logPath = Path.Combine(Application.dataPath, "plc_rtt_log.csv");

        if (!File.Exists(logPath))
        {
            File.WriteAllText(logPath, "Timestamp,RequestID,RTT_ms,SensorData\n");
        }
    ...
    void StartServer()
    {
        try
        {
            ...
            listener.Start();
            isRunning = true;
            logQueue.Enqueue("✅ PLC RTT Server started on port " + port);
            ...
            while (isRunning && client.Connected)
            {
                ...
                if (reply.StartsWith("REQ"))
                {
                    ...
                    if (parts.Length == 2 && requestTimestamps.TryGetValue(parts[0], out DateTime sentTime))
                    {
                        ...
                        logQueue.Enqueue($"PLC RTT ({parts[0]}): {rttMs} ms | Data: {parts[1]}");
            ...
            logQueue.Enqueue("🚪 PLC RTT connection closed.");
        }
        catch (Exception e)
        {
            logQueue.Enqueue("❌ PLC RTT server error: " + e.Message);
        }
    }
}
